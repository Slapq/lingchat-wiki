...
思考相关选项位于 **大模型管理 → 添加模型 / 编辑模型** 的表单中，只有支持思考能力的模型才会显示：

- **启用思考链**：勾选后模型会一并返回思考过程，可在 **对话历史** 中展开查看「思考过程（N 字）」。
- **推理深度**：按模型能力提供 Low / Medium / High 等档位，档位越高模型思考得越多，响应也越慢。
- **关闭思考（追求极速）**：把推理深度设为 Off，或直接切换到更轻量的模型（例如应用内预设「DeepSeek V4 Flash」），无需等待慢速推理。
